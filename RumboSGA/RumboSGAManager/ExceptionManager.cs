    ...
    public static class ExceptionManager
    {
        ...
        public static void GestionarErrorNuevo(Exception ex, String mensajeEspecifico)
        {
            ...
            if (ex is SqlException)
            {
                ...
                else if ((ex as SqlException).Number == 2627 || (ex as SqlException).Number == 2601)
                {
                    String mensaje = Lenguaje.traduce("Se ha intentado insertar un valor con la clave duplicado");
                    RadMessageBox.Show(mensaje);
                }
                else if ((ex as SqlException).Number == 515)
                {
                    String mensaje = Lenguaje.traduce("No se ha insertado el valor en los campos");
                    RadMessageBox.Show(mensaje);
                }
                else if ((ex as SqlException).Number == 207)
                {
                    String mensaje = Lenguaje.traduce(ex.Message);
                    RadMessageBox.Show(mensaje);
                }
                else if ((ex as SqlException).Number == 8152)
                {
                    String mensaje = Lenguaje.traduce("Los datos de cadena tienen una longitud mayor al permitido ");
                    RadMessageBox.Show(mensaje);
                }
            }
            ...
            {
                if (ex.Message.Contains("ArrayIndexOutOfBoundsException"))
                {
                    errorJava = ex.Message.Substring(20);
                    mensajeAMostrar += Lenguaje.traduce("El problema se debe a que la ubicación esta duplicada");
                }
                else
                {
                    errorJava = ex.Message.Substring(20);
                    mensajeAMostrar += Lenguaje.traduce(errorJava);
                    log.Error("Ha generado error " + ex.Message);
                }
            }
            ...
            if (ex.Message.ToLower().Contains("sql") && mensajeEspecifico.ToLower().Contains("sql"))
            {
                log.Error(ex.Message);
                mensajeAMostrar += Lenguaje.traduce("Ha surgido un problema con la conexion en la base de datos ");
            }
