...
namespace RumboSGA
{
    public class FuncionesGenerales
    {
        ...
        public static void exportarAExcelGenerico(RadGridView rgv)
        {
            try
            {
                ...
                for (int i = 0; i < rgv.Columns.Count; i++)
                {
                    ...
                    //rgv.Columns[i].ExcelExportType = DisplayFormatType.;
                }
        ...
        /*public static DataTable descargarJsonDatos(String nombreJson)
        {
            if (SFPrincipal.filtroInicialPersonalizados != null && SFPrincipal.filtroInicialPersonalizados.ContainsKey(nombreJson))
            {
                return descargarJsonDatos(nombreJson, SFPrincipal.filtroInicialPersonalizados[nombreJson]);
            }
            else
            {
                return descargarJsonDatos(nombreJson, "");
            }
        }*/
        ...
        /*public static int cargarEsquemaYSacarCount(string nombreJson, ref List<TableScheme> lstEsquemaTabla)
        {
            try
            {
                DataTable dt = null;
                ...
                lstEsquemaTabla = JsonConvert.DeserializeObject<List<TableScheme>>(jsonEsquemaSelect);

                String filtroInicial = "";

                if (SFPrincipal.filtroInicialPersonalizados != null && SFPrincipal.filtroInicialPersonalizados.ContainsKey(nombreJson))
                {
                    filtroInicial = SFPrincipal.filtroInicialPersonalizados[nombreJson];
                }
                else
                {
                    filtroInicial = "";
