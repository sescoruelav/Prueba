...
{
    public partial class PermisosControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetPermisosCantidad(ref _lstEsquemaTabla);
        ...
        {
            Business.GetPermisosCantidad(ref _lstEsquemaTabla);
            ...
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    GridView.DataSource = Business.GetPermisosDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "PERM." + item.PropertyName;
            ...
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                ...
                numRegistrosFiltrados = Business.GetPermisosRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetPermisosRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetPermisosDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetPermisosRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        {
            return Business.AltaPermiso(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar un Lote
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarPermiso(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un Lote
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarPermiso(_lstEsquemaTabla, selectedRow);
        }
