...
{
    public partial class AgenciasControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetAgenciasCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count!=0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetAgenciasCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetAgenciasDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDAGENCIA ASC";
            }

            if (filterExpression != string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetAgenciasRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetAgenciasRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetAgenciasDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        }
        ...
        private void gridViewFilterChanged_Event(object sender,EventArgs e)
        {
            ...
            {           
             lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetAgenciasRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
            }
        }
