...
{
    public partial class FamiliasControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetFamiliasCantidad(ref _lstEsquemaTabla);
        ...
        public override void btnCambiarVista_Click(object sender, EventArgs e)
        {
            ...
            else if (TblLayout.GetControlFromPosition(0, 0) is RadVirtualGrid)
            {
                ...
                else
                {
                    ...
                    TblLayout.Controls.Remove(virtualGridControl);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetFamiliasCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetFamiliasDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "FAM." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDFAMILIA ASC";
            }
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetFamiliasRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetFamiliasRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }

            this.virtualGridControl.MasterViewInfo.IsWaiting = true;

            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetFamiliasDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        }
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = strings.NProductos + ":" + Business.GetFamiliasRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
