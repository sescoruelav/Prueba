...
{
    public partial class RutasControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetRutasCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetRutasCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetRutasDatosGridView(_lstEsquemaTabla);
                    foreach (var column in GridView.Columns)
                    {
                            column.Name = _lstEsquemaTabla[column.Index].Nombre;
                            column.FieldName = _lstEsquemaTabla[column.Index].Etiqueta;
                            column.HeaderText = _lstEsquemaTabla[column.Index].Etiqueta;
                    }
                }
                catch (Exception e)
                {
        ...
        //Función que, asíncronamente, consulta los datos de Rutas de la página seleccionada en el Grid
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "RUTAS." + item.PropertyName;
            ...
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                ...
                numRegistrosFiltrados = Business.GetRutasRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetRutasRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }

            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetRutasDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetRutasRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        //Acción para crear una Ruta
        protected override AckResponse NewData(dynamic selectedRow)
        {
            return Business.AltaRuta(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar una Ruta
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarRuta(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar una Ruta
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarRuta(_lstEsquemaTabla, selectedRow);
        }
