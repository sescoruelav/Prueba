...
{
    public partial class IncidenciasInventarioControl : BaseGridControl
    {
        ...
        public IncidenciasInventarioControl()
        {
            InitializeComponent();
        ...
        {
            //Obtenemos, asíncronamente, la cantidad total de registros de Clientes, así como su esquema de datos, para poder preparar el Grid                        
            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            CantidadRegistros = Business.GetIncidenciasInventarioCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetIncidenciasInventarioCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetIncidenciasInventarioDatosGridView(_lstEsquemaTabla);
        ...
        //Función que, asíncronamente, consulta los datos de Clientes de la página seleccionada en el Grid
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "INCIDENCIASINV." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDINCIDENCIA ASC";
            }
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetIncidenciasInventarioRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetIncidenciasInventarioRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetIncidenciasInventarioDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetIncidenciasInventarioRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        {
            return Business.AltaIncidenciaInventario(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar un Cliente
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarIncidenciaInventario(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un Cliente
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarIncidenciaInventario(_lstEsquemaTabla, selectedRow);
        }
