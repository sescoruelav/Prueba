...
{
    public partial class UsuariosGruposControl : BaseGridControl
    {
        ...
        {
            //Obtenemos, asíncronamente, la cantidad total de registros de UsuariosGrupo, así como su esquema de datos, para poder preparar el Grid                        
            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            CantidadRegistros = Business.GetUsuariosGruposCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetUsuariosGruposCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetUsuariosGruposDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "USUGRUP." + item.PropertyName;
            ...
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                ...
                numRegistrosFiltrados = Business.GetUsuariosGruposRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetUsuariosGruposRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }
            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetUsuariosGruposDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        }
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetUsuariosGruposRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        //Acción para crear un UsuarioGrupo
        protected override AckResponse NewData(dynamic selectedRow)
        {
            return Business.AltaUsuarioGrupo(_lstEsquemaTabla, selectedRow);
        }
        //Acción para editar un UsuarioGrupo
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarUsuarioGrupo(_lstEsquemaTabla, selectedRow);
        }
        //Acción para eliminar un UsuarioGrupo
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarUsuarioGrupo(_lstEsquemaTabla, selectedRow);
        }
        #endregion
    }
}