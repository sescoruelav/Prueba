...
{
    public partial class PaletsTipoControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetPaletsTipoCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            try
            {
                ...
                GridView.DataSource = Business.GetPaletsTipoDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "PALETSTIPO." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDPALETTIPO ASC";
            }
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetPaletsTipoRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetPaletsTipoRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetPaletsTipoDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetPaletsTipoRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        //Acción para crear un PaletTipo
        protected override AckResponse NewData(dynamic selectedRow)
        {
            return Business.AltaPaletTipo(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar un PaletTipo
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarPaletTipo(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un PaletTipo
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarPaletTipo(_lstEsquemaTabla, selectedRow);
        }
