...
{
    public partial class EstadoRecepcionMotor : BaseGridControl
    {
        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        RadColorDialog dialog = new RadColorDialog();

        private static String nombreJson = "EstadoRecepcion";
        ...
        public EstadoRecepcionMotor()
        {
            try
            {
                ...
                {
                    name = "Estado Recepcion";
                }
                else
                {
                    name = "Recepcion Status";
                }
            }
            catch (Exception ex)
            {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetDatosCantidad(ref _lstEsquemaTabla,nombreJson);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetDatosCantidad(ref _lstEsquemaTabla,nombreJson);
                    GridView.DataSource = Business.GetDatosGridView(_lstEsquemaTabla,nombreJson, "IDRECEPCIONESTADO");
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            numRegistrosFiltrados = Business.GetDatosRegistrosFiltrados("",nombreJson);
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetDatos(nombreJson,"", "TBLRECEPCIONESTADO", "", "", pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
            ColorEstado();
        }
        ...
        private void ColorEstado()
        {
            ...
            if (Business.getEstadoRecepcionCondicionalesColor(ref listaCondicionales) > 0)
            {
