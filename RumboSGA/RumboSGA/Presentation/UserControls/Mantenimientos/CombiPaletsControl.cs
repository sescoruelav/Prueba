...
{
    public partial class CombiPaletsControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetCombiPaletsCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetCombiPaletsCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetCombiPaletsDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "COMBIPALETS." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "TIPOANCHOA ASC";
            }
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetCombiPaletsRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetCombiPaletsRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetCombiPaletsDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        }
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetCombiPaletsRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        //Acción para crear un CombiPalet
        protected override AckResponse NewData(dynamic selectedRow)
        {
            return Business.AltaCombiPalet(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar un CombiPalet
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarCombiPalet(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un CombiPalet
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarCombiPalet(_lstEsquemaTabla, selectedRow);
        }
