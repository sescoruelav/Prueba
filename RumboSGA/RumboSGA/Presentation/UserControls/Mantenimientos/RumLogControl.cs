...
{
    public partial class RumLogControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetRumLogCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetRumLogCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetRumLogDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "RUMLOG." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDLOGOPERACIONES ASC";
            }
            if (filterExpression!=null&&filterExpression!=string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetRumLogRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetRumLogRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }

            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetRumLogDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        }
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetRumLogRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
