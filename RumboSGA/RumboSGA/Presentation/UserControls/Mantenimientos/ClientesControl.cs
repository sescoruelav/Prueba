...
{
    public partial class ClientesControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetClientesCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetClientesCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetClientesDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "CLI." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDCLIENTE ASC";
            }
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetClientesRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetClientesRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetClientesDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        }
        ...
        protected override void RadVirtualGrid_CellValueNeeded(object sender, VirtualGridCellValueNeededEventArgs e)
        {
            ...
            if (e.RowIndex >= 0 && data.Count > 0)
            {
                ...
                try
                {
                    ...
                    var z = JsonConvert.DeserializeObject(Convert.ToString(this.data[index]));
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetClientesRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        {
            return Business.AltaCliente(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar un Cliente
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarCliente(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un Cliente
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarCliente(_lstEsquemaTabla, selectedRow);
        }
