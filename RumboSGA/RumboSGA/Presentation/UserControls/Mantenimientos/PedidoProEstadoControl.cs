...
{
    public partial class PedidoProEstadoControl : BaseGridControl
    {
        ...
        private void GridView_ContextMenuOpening(object sender, ContextMenuOpeningEventArgs e)
        {
            try
            {
                if (this.GridView.CurrentRow is GridViewDataRowInfo)
                {
                    if (CultureInfo.CurrentUICulture.Name == "es-ES")
                    {
                        if (GridView.CurrentColumn.Name == "IDPEDIDOPROESTADO")
                        {
        ...
        private void menu_ItemClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                XmlReaderPropio.setColorPedidos(color, estado);
                Reload();
            }
            catch (Exception ex)
            {
        ...
        private void Reload()
        {
            try
            {
                Business.GetEstadoPedidoControlCantidad(ref _lstEsquemaTabla);
                GridView.DataSource = Business.GetEstadoPedidoControlDatosGridView(_lstEsquemaTabla);
                this.Dock = System.Windows.Forms.DockStyle.Fill;
                GridView.ReadOnly = true;
                GridView.BestFitColumns();
                this.GridView.ContextMenuOpening += GridView_ContextMenuOpening;
        ...
        private void ColorPedidosPro()
        {
            try
            {
                ...
                this.GridView.MasterTemplate.Columns["IDPEDIDOPROESTADO"].ConditionalFormattingObjectList.Add(obj1);
                this.GridView.MasterTemplate.Columns["IDPEDIDOPROESTADO"].ConditionalFormattingObjectList.Add(obj2);
                this.GridView.MasterTemplate.Columns["IDPEDIDOPROESTADO"].ConditionalFormattingObjectList.Add(obj3);
                this.GridView.MasterTemplate.Columns["IDPEDIDOPROESTADO"].ConditionalFormattingObjectList.Add(obj4);
                this.GridView.MasterTemplate.Columns["IDPEDIDOPROESTADO"].ConditionalFormattingObjectList.Add(obj5);
            }
            catch (Exception ex)
            {
                //ExceptionManager.GestionarError(ex);
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetEstadoPedidoControlCantidad(ref _lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDPEDIDOPROESTADO ASC";
            }

            if (filterExpression != string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetEstadoPedidoControlRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetEstadoPedidoControlRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetEstadoPedidoControlDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
            ColorPedidosPro();
        }
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetEstadoPedidoControlRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
            }
        }
        ...
        {
            return Business.AltaEstadoPedidoControl(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar una Agencia
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarEstadoPedidoControl(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar una Agencia
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarEstadoPedidoControl(_lstEsquemaTabla, selectedRow);
        }
