...
{
    public partial class GruposControl : BaseGridControl
    {
        ...
        public GruposControl()
        {
            ...
            //ElegirEstilo();
            RefreshData(0);
            //virtualGridControl.AutoSize = true;
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetGruposCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetGruposCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = null;
                    GridView.DataSource = Business.GetGruposDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression +="MAQ."+item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDGRUPO ASC";
            }
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                lblCantidad.Text= Lenguaje.traduce(strings.NProductos) + ":" + Business.GetGruposRegistrosFiltrados(filterExpression);
                numRegistrosFiltrados = Business.GetGruposRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetGruposCantidad(ref _lstEsquemaTabla);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }
            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetGruposDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        }
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetGruposRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
            }
        }
