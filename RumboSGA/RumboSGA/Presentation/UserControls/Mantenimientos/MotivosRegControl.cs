...
{
    public partial class MotivosRegControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetMotivosRegCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetMotivosRegCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetMotivosRegDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "MOTREG." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDMOTIVO ASC";
            }
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetMotivosRegRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetMotivosRegRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetMotivosRegDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetMotivosRegRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        //Acción para crear un MotivosReg
        protected override AckResponse NewData(dynamic selectedRow)
        {
            return Business.AltaMotivosReg(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar un MotivosReg
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarMotivosReg(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un MotivosReg
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarMotivosReg(_lstEsquemaTabla, selectedRow);
        }
