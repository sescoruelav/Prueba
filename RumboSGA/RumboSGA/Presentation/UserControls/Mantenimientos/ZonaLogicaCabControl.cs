...
{
    public partial class ZonaLogicaCabControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetZonaLogCabCantidad(ref _lstEsquemaTabla, ref _esquemGrid);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetZonaLogCabCantidad(ref _lstEsquemaTabla, ref _esquemGrid);
                    GridView.DataSource = Business.GetZonaLogCabDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "ZONACAB." + item.PropertyName;
            ...
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                ...
                numRegistrosFiltrados = Business.GetZonaLogCabRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetZonaLogCabRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }
            ...
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetZonaLogCabDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        }
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetZonaLogCabRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        //Acción para crear una Zona
        protected override AckResponse NewData(dynamic selectedRow)
        {
            return Business.AltaZonaCab(_lstEsquemaTabla, selectedRow);
        }

        /*protected override AckResponse NewDataAndLines(dynamic selectedRow, bool bolCloneLines)
        {
            return Business.AltaZonaCabYLineas(_lstEsquemaTabla, selectedRow, _esquemGrid);
  
        }*/

        //Acción para editar una Zona
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarZonaCab(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un Zona
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarZonaCab(_lstEsquemaTabla, selectedRow);
        }
