...
{
    public partial class OperariosMovControl : BaseGridControl
    {
        ...
        {
            //Obtenemos, asíncronamente, la cantidad total de registros de OperariosMovimientos, así como su esquema de datos, para poder preparar el Grid                        
            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            CantidadRegistros = Business.GetOperariosMovCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetOperariosMovCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetOperariosMovDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "OPERMOV." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDMOVIMIENTO ASC";
            }
            if (filterExpression!=null&&filterExpression!=string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetOperariosMovRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetOperariosMovRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }

            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetOperariosMovDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        }
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetOperariosMovRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        //Acción para crear un OperariosMovimientos
        protected override AckResponse NewData(dynamic selectedRow)
        {
            return Business.AltaOperariosMov(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar un OperariosMovimientos
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarOperariosMov(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un OperariosMovimientos
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarOperariosMov(_lstEsquemaTabla, selectedRow);
        }
