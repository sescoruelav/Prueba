...
{
    public partial class RutasPreparacionControl : BaseGridControl
    {
        ...
        public RutasPreparacionControl()
        {
            InitializeComponent();
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetRutasPreparacionCantidad(ref _lstEsquemaTabla);
        ...
        protected override void getGridViewRow()
        {
            for (int i = 0; i < data.Count; i++)
            {
                ...
                {
                    var row = this.virtualGridControl.CurrentCell.RowIndex;
                    if (data[row][_lstEsquemaTabla[0].Nombre].Value.ToString() == jsonID)
                    {
                        _selectedRow = data[i];
                        break;
                    }
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetRutasPreparacionCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetRutasPreparacionDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "RUTASPREP." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDRUTA ASC";
            }
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetRutasPreparacionRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetRutasPreparacionRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetRutasPreparacionDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetRutasPreparacionRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        //Acción para crear una RutaPreparacion
        protected override AckResponse NewData(dynamic selectedRow)
        {
            return Business.AltaRutaPreparacion(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar una RutaPreparacion
        protected override AckResponse EditData(dynamic selectedRow)
        {
            AckResponse ack= Business.EditarRutaPreparacion(_lstEsquemaTabla, selectedRow);
            try
            {
                ...
                if (ack.Resultado.Equals("OK"))
                {
                    ...
                    if (valores1["IDRUTAESTADO"].ToString().Equals("'CE'"))
                    {


                        WSPedidoCliMotorClient ws = new WSPedidoCliMotorClient();
                        ws.cerrarRutaPreparacion(Convert.ToInt32(valores1["IDRUTA"]), valores1["DIA"].ToString(), DatosThread.getInstancia().getArrayDatos());
        ...
        //Acción para eliminar una RutaPreparacion
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarRutaPreparacion(_lstEsquemaTabla, selectedRow);
        }
