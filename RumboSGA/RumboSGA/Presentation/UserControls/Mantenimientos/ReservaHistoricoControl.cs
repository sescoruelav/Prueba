...
{
    public partial class ReservaHistoricoControl : BaseGridControl
    {
        ...
        public ReservaHistoricoControl()
        {
            InitializeComponent();
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetReservasHistoricoCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                try
                {
                    ...
                    GridView.DataSource = Business.GetReservasHistoricoDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "RESHIST." + item.PropertyName;
            ...
            if (filterExpression!=null&&filterExpression != string.Empty)
            {
                ...
                numRegistrosFiltrados = Business.GetReservasHistoricoRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetReservasHistoricoRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }

            this.virtualGridControl.MasterViewInfo.IsWaiting = true;

            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetReservasHistoricoDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        }
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetReservasHistoricoRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        {
            return Business.AltaReservaHistorico(_lstEsquemaTabla, selectedRow);            
        }

        //Acción para editar un ReservaHistorico
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarReservaHistorico(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un ReservasHistorico
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarReservaHistorico(_lstEsquemaTabla, selectedRow);            
        }
