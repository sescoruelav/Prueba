...
{
    public partial class ParametrosControl : BaseGridControl
    {
        ...
        {
            //Obtenemos, asíncronamente, la cantidad total de registros de CambiarAtributos, así como su esquema de datos, para poder preparar el Grid                        
            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            CantidadRegistros = Business.GetParametrosCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                try
                {
                    ...
                    GridView.DataSource = Business.GetParametrosDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "PARAM." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "PARAMETRO ASC";
            }
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetParametrosRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetParametrosRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }

            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetParametrosDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetParametrosRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        //Acción para crear un Parametro
        protected override AckResponse NewData(dynamic selectedRow)
        {
            return Business.AltaParametro(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar un Parametro
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarParametro(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un Parametro
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarParametro(_lstEsquemaTabla, selectedRow);
        }
