...
{
    public partial class ZonaIntercambioControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetZonaIntercambioCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetZonaIntercambioCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetZonaIntercambioDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "ZON." + item.PropertyName;
            ...
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                ...
                numRegistrosFiltrados = Business.GetZonaIntercambioRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetZonaIntercambioRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetZonaIntercambioDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetZonaIntercambioRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        {
            return Business.AltaZonaIntercambio(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar una ZonaIntercambio
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarZonaIntercambio(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar una ZonaIntercambio
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarZonaIntercambio(_lstEsquemaTabla, selectedRow);
        }
