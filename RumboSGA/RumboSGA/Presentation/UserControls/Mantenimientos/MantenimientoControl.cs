...
{
    public partial class MantenimientoControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetMantenimientoCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetMantenimientoCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetMantenimientoDatosGridView(_lstEsquemaTabla);
        ...
        //Función que, asíncronamente, consulta los datos de Mantenimiento de la página seleccionada en el Grid
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "MANT." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDMANTENIMIENTO ASC";
            }
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetMantenimientoRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetMantenimientoRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }

            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetMantenimientoDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetMantenimientoRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        //Acción para crear un Mantenimiento
        protected override AckResponse NewData(dynamic selectedRow)
        {
            return Business.AltaMantenimiento(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar un Mantenimiento
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarMantenimiento(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un Mantenimiento
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarMantenimiento(_lstEsquemaTabla, selectedRow);
        }
