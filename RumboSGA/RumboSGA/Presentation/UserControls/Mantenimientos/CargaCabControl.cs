...
{
    public partial class CargaCabControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetCargaCabCantidad(ref _lstEsquemaTabla);
        ...
        protected override void getGridViewRow()
        {
            ...
            log.Info("Numero de filas=" + data.Count);
            for (int i = 0; i < data.Count; i++)
            {
                JObject objetoJson = JObject.Parse(data[i].ToString());
                string jsonID = (string)objetoJson[_lstEsquemaTabla[0].Nombre];
                log.Info(i + " " + _lstEsquemaTabla[0].Nombre + " valor=" + this.GridView.CurrentRow.Cells[_lstEsquemaTabla[0].Nombre].Value + " jsonid=" + jsonID);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetCargaCabCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetCargaCabDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "CARGACAB." + item.PropertyName;
            ...
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetCargaCabDatos(sortExpression, filterExpression, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetCargaCabRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        {
            return Business.AltaCarga(_lstEsquemaTabla, selectedRow);
        }
        ...
        {
            return Business.EditarCarga(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un Pedido
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarCarga(_lstEsquemaTabla, selectedRow);
        }
