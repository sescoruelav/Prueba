...
{
    public partial class PropietariosControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetPropietariosCantidad(ref _lstEsquemaTabla);
        ...
        public override void btnCambiarVista_Click(object sender, EventArgs e)
        {
            ...
            if (TblLayout.GetControlFromPosition(0, 0) is RadGridView)
            {
                ...
                //btnExportacion.Visible = false;

                //  baseGrid.gridViewArticulos.SaveLayout(topControl1.ViewLabel.Text + ".xml");
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetPropietariosCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetPropietariosDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "PROP." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDPROPIETARIO ASC";
            }
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetPropietariosRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetPropietariosRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }

            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetPropietariosDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetPropietariosRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
            }
        }
        ...
        {
            return Business.AltaPropietario(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar una Agencia
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarPropietario(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar una Agencia
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarPropietario(_lstEsquemaTabla, selectedRow);
        }
