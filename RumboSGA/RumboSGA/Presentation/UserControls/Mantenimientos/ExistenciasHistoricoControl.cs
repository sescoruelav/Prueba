...
{
    public partial class ExistenciasHistoricoControl : BaseGridControl
    {
        ...
        public ExistenciasHistoricoControl()
        {
            InitializeComponent();
            InicializarEventos();
            ...
            ElegirGrid();
            //ElegirEstilo();
            BaseGridControl.nombreJson = "ExistenciasHistorico";
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            var CantidadTotal = Business.GetExistenciasHistoricoCantidad(ref _lstEsquemaTabla);
            /*if (SFPrincipal.filtroInicialPersonalizados.ContainsKey(SFPrincipal.jsonFiltroInicial))
            {
                if (!string.IsNullOrEmpty(SFPrincipal.filtroInicialPersonalizados[SFPrincipal.jsonFiltroInicial]))
                {
                    CantidadRegistros = Business.GetExistenciasHistoricoRegistrosFiltrados("WHERE " + SFPrincipal.filtroInicialPersonalizados[SFPrincipal.jsonFiltroInicial], _lstEsquemaTabla, true);
                }
            }
            else*/
                CantidadRegistros = Business.GetExistenciasHistoricoRegistrosFiltrados("", _lstEsquemaTabla);
        ...
        protected override void getGridViewRow()
        {
            ...
            if(this.Controls[0].Controls[0] is RadVirtualGrid)
            {
                _selectedRow = null;
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetExistenciasHistoricoCantidad(ref _lstEsquemaTabla);
                    /*if (SFPrincipal.filtroInicialPersonalizados.ContainsKey(SFPrincipal.jsonFiltroInicial))
                    {
                        if (!string.IsNullOrEmpty(SFPrincipal.filtroInicialPersonalizados[SFPrincipal.jsonFiltroInicial]))
                            GridView.DataSource = Business.GetExistenciasHistoricoDatosGridView(_lstEsquemaTabla, " WHERE " +
                                SFPrincipal.filtroInicialPersonalizados[SFPrincipal.jsonFiltroInicial]);
                    }
                    else*/
                        GridView.DataSource = Business.GetExistenciasHistoricoDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "EXISTHIST." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDHISTORICO ASC";
            }
            /*if (SFPrincipal.filtroInicialPersonalizados.ContainsKey(SFPrincipal.jsonFiltroInicial))
            {
                if (!string.IsNullOrEmpty(SFPrincipal.filtroInicialPersonalizados[SFPrincipal.jsonFiltroInicial]))
                {
                    numRegistrosFiltrados = Business.GetExistenciasHistoricoRegistrosFiltrados("WHERE " + SFPrincipal.filtroInicialPersonalizados[SFPrincipal.jsonFiltroInicial], _lstEsquemaTabla, true);
            ...
            {*/
                if (filterExpression != null && filterExpression != string.Empty)
                {
                    ...
                    numRegistrosFiltrados = Business.GetExistenciasHistoricoRegistrosFiltrados(filterExpression, _lstEsquemaTabla);
                    lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

                }
                else
                {
                    numRegistrosFiltrados = Business.GetExistenciasHistoricoRegistrosFiltrados("", _lstEsquemaTabla);
                    lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
                }
            //}
            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ...
            {
                if (!string.IsNullOrEmpty(SFPrincipal.filtroInicialPersonalizados[SFPrincipal.jsonFiltroInicial]))
                    ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetExistenciasHistoricoDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla, " WHERE " + SFPrincipal.filtroInicialPersonalizados[SFPrincipal.jsonFiltroInicial])), this.Callback);
            }
            else*/
                ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetExistenciasHistoricoDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
        }
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetExistenciasHistoricoRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression, _lstEsquemaTabla).ToString();
        ...
        //Acción para crear un ExistenciasHistorico
        protected override AckResponse NewData(dynamic selectedRow)
        {
            return Business.AltaExistenciasHistorico(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar un ExistenciasHistorico
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarExistenciasHistorico(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un ExistenciasHistorico
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarExistenciasHistorico(_lstEsquemaTabla, selectedRow);
        }
