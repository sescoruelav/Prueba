...
{
    public partial class FormatosSCCControl : BaseGridControl
    {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetFormatosSCCCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetFormatosSCCCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetFormatoSSCCDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            {
                sortExpression += "FORMATOSSCC." + item.PropertyName;
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDFORMATO ASC";
            }
            if (filterExpression != null&&filterExpression!=string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetFormatosSCCRegistrosFiltrados(filterExpression);
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetFormatosSCCRegistrosFiltrados("");
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + numRegistrosFiltrados;
            }


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetFormatoSSCCDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        }
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
            lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetFormatosSCCRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
        ...
        //Acción para crear un FormatoSCC
        protected override AckResponse NewData(dynamic selectedRow)
        {
            return Business.AltaFormatoSCC(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar un FormatoSCC
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarFormatoSCC(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar un FormatoSCC
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarFormatoSCC(_lstEsquemaTabla, selectedRow);
        }
