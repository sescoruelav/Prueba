...
{
    public partial class EstadoFabricacionControl : BaseGridControl
    {
        ...
        #endregion
        public EstadoFabricacionControl()
        {
            try
            {
                InitializeComponent();
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetEstadoFabricacionControlCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetEstadoFabricacionControlCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetEstadoFabricacionControlDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDORDENFABESTADO ASC";
            }

            if (filterExpression != string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetEstadoFabricacionControlRegistrosFiltrados(filterExpression);
                lblCantidad.Text = ""+numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetEstadoFabricacionControlRegistrosFiltrados("");
                lblCantidad.Text =  ""+numRegistrosFiltrados;
            }


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetEstadoFabricacionControlDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        }
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetEstadoFabricacionControlRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
            }
        }
        ...
        {
            return Business.AltaEstadoFabricacionControl(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar una Agencia
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarEstadoFabricacionControl(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar una Agencia
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarEstadoFabricacionControl(_lstEsquemaTabla, selectedRow);
        }
