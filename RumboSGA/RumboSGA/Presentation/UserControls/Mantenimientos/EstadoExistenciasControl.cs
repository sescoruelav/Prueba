...
{
    public partial class EstadoExistenciasControl : BaseGridControl
    {
        ...
        #endregion
        public EstadoExistenciasControl()
        {
            try
            {
                InitializeComponent();
                ...
                ColorEstado();
                name = Lenguaje.traduce("Estado Existencia");
            }
            catch (Exception ex)
            {
        ...
        private void ObtencionTotalRegistros()
        {
            ...
            CantidadRegistros = Business.GetEstadoExistenciasControlCantidad(ref _lstEsquemaTabla);
        ...
        public override void llenarGrid()
        {
            if (_lstEsquemaTabla.Count != 0)
            {
                ...
                {
                    base.llenarGrid();
                    Business.GetEstadoExistenciasControlCantidad(ref _lstEsquemaTabla);
                    GridView.DataSource = Business.GetEstadoExistenciasControlDatosGridView(_lstEsquemaTabla);
        ...
        public override void RefreshData(int pageIndex)
        {
            ...
            if (sortExpression == string.Empty)
            {
                sortExpression = "IDEXISTENCIAESTADO ASC";
            }

            if (filterExpression != string.Empty)
            {
                filterExpression = " WHERE " + filterExpression;
                numRegistrosFiltrados = Business.GetEstadoExistenciasRegistrosFiltrados(filterExpression);
                lblCantidad.Text = "" + numRegistrosFiltrados;

            }
            else
            {
                numRegistrosFiltrados = Business.GetEstadoExistenciasRegistrosFiltrados("");
                lblCantidad.Text = "" + numRegistrosFiltrados;
            }


            this.virtualGridControl.MasterViewInfo.IsWaiting = true;
            ExecuteQueryAsync<List<dynamic>>(Task.Run(() => Business.GetEstadoExistenciasControlDatos(sortExpression, filterExpression, pagInicial, pagFinal, _lstEsquemaTabla)), this.Callback);
            ElegirEstilo();
        }
        ...
        private void gridViewFilterChanged_Event(object sender, EventArgs e)
        {
            ...
            {
                lblCantidad.Text = Lenguaje.traduce(strings.NProductos) + ":" + Business.GetEstadoExistenciasRegistrosFiltrados(" WHERE " + GridView.FilterDescriptors.Expression).ToString();
            }
        }
        ...
        {
            return Business.AltaEstadoExistenciasControl(_lstEsquemaTabla, selectedRow);
        }

        //Acción para editar una Agencia
        protected override AckResponse EditData(dynamic selectedRow)
        {
            return Business.EditarEstadoExistenciasControl(_lstEsquemaTabla, selectedRow);
        }

        //Acción para eliminar una Agencia
        protected override AckResponse DeleteData(dynamic selectedRow)
        {
            return Business.EliminarEstadoExistenciasControl(_lstEsquemaTabla, selectedRow);
        }
        ...
        {
            //Voy a hacer un columns remove para no mostrar el COLORACAMBIAR
            if (this.GridView.ColumnCount > 2)
            {
                this.GridView.Columns.RemoveAt(this.GridView.ColumnCount - 1);
            }
            if (this.GridView.Columns[0].ConditionalFormattingObjectList.Count > 0)
            {
                this.GridView.Columns[0].ConditionalFormattingObjectList.Clear();
            }
            List<ConditionalFormattingObject> listaCondicionales = new List<ConditionalFormattingObject>();
            if (Business.getEstadoExistenciasCondicionalesColor(ref listaCondicionales) > 0)
            {
                foreach (ConditionalFormattingObject item in listaCondicionales)
                {
                    this.GridView.Columns[0].ConditionalFormattingObjectList.Add(item);
                }
            }
        }
        #endregion
