...
{
    public partial class PorArticuloFechaRegSalida : Telerik.WinControls.UI.RadForm
    {
        ...
        RadGridView gridRegulaSalida;
        
        public PorArticuloFechaRegSalida(RadGridView gridRegulaSalida)
        {
            InitializeComponent();
            this.gridRegulaSalida = gridRegulaSalida;
            ...
            this.Text = Lenguaje.traduce("Regularización Salida por Artículo y Fecha");
            this.ShowIcon = false;
        ...
        {
            RegulaSalida.q = ConfigurarSQL.CargarConsulta(path, "REGULARIZACIONESSALIDA") + " WHERE S.IDSALIDATIPO = 'SR' AND(REPLACE(left(CONVERT(varchar(10), S.FECHA, 126), 10), '-', '') >= '" + validarFecha(dateTimePicker1) + "' AND REPLACE(left(CONVERT(varchar(10), S.FECHA, 126), 10), '-', '') <= '" + validarFecha(dateTimePicker2) + "' AND S.IDMOTIVO = '" + motivosList.SelectedRows[0].Cells[0].Value + "')";
            gridRegulaSalida.DataSource = Utilidades.TraducirDataTableColumnNameDT(ConexionSQL.getDataTable(RegulaSalida.q)).DefaultView;
            this.Close();
        }
