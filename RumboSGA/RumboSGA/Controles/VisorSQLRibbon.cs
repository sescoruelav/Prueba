...
namespace RumboSGA.Controles
{
    public partial class VisorSQLRibbon : Telerik.WinControls.UI.RadRibbonForm
    {
            ...
            switch (Estado_)
            {
                ...
                case Estados.VCTodoDetalle:
                    ...
                    else
                    {
                        ...
                        panelDatos = cargaTablaDetalle.CargaTablaDetalle(name, lstEsquemaTabla, selectedRow, false, false, tablaPrincipalJson, this.tablaPrincipal.Width);
        ...
        private void insertarVirtualGrid()
        {
            ...
            int a = virtualGrid.RowCount;
            log.Info("Info columna virtual insert " + a);
        ...
        private void cargarVirtualGrid()
        {
            if (virtualGrid == null)
            {
                ...
                //virtualGrid.d += radVirtualGrid1_CellFormatting;
                ;
        ...
        private void radVirtualGrid1_CellValueNeeded(object sender, VirtualGridCellValueNeededEventArgs e)
        {
            ...
            {
                if (e.RowIndex < dtFiltradoVirtual.Rows.Count)
                {
        ...
        private void VirtualGridFiltering_Load(object sender, EventArgs e)
        {
            ...
            this.virtualGrid.FilterChanged += radVirtualGrid1_FilterChanged;
        }

        private void filtradoVirtualGrid()
        {
            if (dtVirtualGrid == null) return;
            string ex = this.virtualGrid.FilterDescriptors.Expression;
            string sort = this.virtualGrid.SortDescriptors.Expression;
            try
            {
                DataTable dt1 = new DataTable();
                if (!string.IsNullOrWhiteSpace(ex))
                {
                    dtVirtualGrid = FuncionesGenerales.descargarJsonDatos(nombreJson, "");
                    for (int i = 0; i < virtualGrid.FilterDescriptors.Count; i++)
                    {
                        if (!string.IsNullOrWhiteSpace(virtualGrid.FilterDescriptors[i].Value.ToString()))
                        {
                            var rows = dtVirtualGrid.AsEnumerable().Where(dato => Convert.ToString(dato[virtualGrid.FilterDescriptors[i].PropertyName.ToString()]).ToUpper().Contains(virtualGrid.FilterDescriptors[i].Value.ToString().ToUpper()));
                            dt1 = rows.CopyToDataTable();
                            dtVirtualGrid = dt1;
                            dtFiltradoVirtual = dtVirtualGrid;
                        }
                    }
                }
                else
                {
                    dtVirtualGrid = FuncionesGenerales.descargarJsonDatos(nombreJson, ex);
                    dtFiltradoVirtual = dtVirtualGrid;
                }

                if (virtualGrid.FilterDescriptors.Count == 1)
                {
                    if (!string.IsNullOrWhiteSpace(ex) && string.IsNullOrWhiteSpace(virtualGrid.FilterDescriptors[0].Value.ToString()))
                    {
                        dtVirtualGrid = FuncionesGenerales.descargarJsonDatos(nombreJson, "");
                        dtFiltradoVirtual = dtVirtualGrid;
                    }
                ...
                this.virtualGrid.RowCount = dtVirtualGrid.Rows.Count;

                lblCantidad.Text = "     " + Lenguaje.traduce("Registros: ") + virtualGrid.RowCount;
            }
            catch (Exception e)
            {
                ExceptionManager.GestionarErrorNuevo(e);
                MessageBox.Show("No se ha encontrado el valor:" + virtualGrid.FilterDescriptors[0].Value.ToString());
            }
        }
        ...
        {
            int b = virtualGrid.ColumnCount;
            log.Info("Info columna virtual alter " + b);

            if (go == GridOpciones.GridView)
            {
                cargarGridView();
            ...
            else if (go == GridOpciones.VirtualGrid)
            {
                cargarVirtualGrid();
                insertarVirtualGrid();
        ...
        private void radVirtualGrid1_CellFormatting(object sender, Telerik.WinControls.UI.VirtualGridCellElementEventArgs e)
        {
            ...
            if (e.CellElement.ColumnIndex >= 0)
            {
                ...
                if (e.CellElement.GetType().FullName.ToString() == "System.Int32")
                {
                    // e.CellElement.Style. = "000.000.000";
                }
                contadorListaTipos++;
            }
            else
        ...
        {
            return RumboSGA.FuncionesGenerales.jsonDelete(nombreJson, RumboSGA.FuncionesGenerales.ComponerJsonDelete(lstEsquemaTabla, this.selectedRow));
        }
        ...
        public void ExportarExcel()
        {
            try
            {
                ...
                {
                    RadMessageBox.Show(Lenguaje.traduce("Opción solo disponible en vista avanzada"));
                    return;
                }
        ...
        public void SaveLayout(string path)
        {
            ...
            try
            {
                ...
                {
                    this.virtualGrid.XmlSerializationInfo.SerializationMetadata.Add(typeof(VirtualGridViewInfo), "CustomColumns", new DesignerSerializationVisibilityAttribute(DesignerSerializationVisibility.Visible));
        ...
        protected virtual void loadData()
        {
            ...
            try
            {
                ...
                int b = virtualGrid.ColumnCount;
                log.Info("Info columna virtual loadData " + b);
        ...
        private void backGroudWorkerDescargaDatos_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            {
                int b = virtualGrid.ColumnCount;
                log.Info("Info columna virtual RunWorkerCompleted " + b);
                if (e.Error != null)
                {
                ...
                log.Debug("Info RunWorkerCompleted");
