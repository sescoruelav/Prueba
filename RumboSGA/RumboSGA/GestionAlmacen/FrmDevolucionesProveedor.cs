...
{
    public partial class FrmDevolucionesProveedor : Telerik.WinControls.UI.RadRibbonForm
    {

        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        private static readonly string nombreJson = "DevolucionesProveedor";
        private static readonly string nombreJsonJerLineas = "DevolucionesProveedorJerarquicoLineas";
        private static readonly string nombreJsonJerEntradas = "DevolucionesProveedorJerarquicoEntradas";
        private static readonly string nombreJsonJerExistencias = "DevolucionesProveedorJerarquicoExistencias";
        private static readonly string nombreJsonJerReservas = "DevolucionesProveedorJerarquicoReservas";
        private static readonly string nombreJsonJerSalidas = "DevolucionesProveedorJerarquicoSalidas";
        ...
        int idDevolProLinSeleccionado = -1;
        ...
        private static int jerarquicoPosicionSalidas = 4;
       ...
       /* ProgressBarColumn colPedidosPorcentLineas;
        ...
        public FrmDevolucionesProveedor()
        {
            try
            {
                InitializeComponent();
        ...
        private void FrmDevolucionesProveedor_Shown(object sender, EventArgs e)
        {
        ...
        private GridViewTemplate CreateChildTemplateJerarquicoSalidas()
        {
            ...
            string jsonLineas = DataAccess.LoadJson(nombreJsonJerSalidas);
            JArray jsLineas = JArray.Parse(jsonLineas);
            string jsonEsquemaLineas = jsLineas.First()["Scheme"].ToString();
            List<TableScheme> esquemaLineas = JsonConvert.DeserializeObject<List<TableScheme>>(jsonEsquemaLineas);
        ...
        private void FrmDevolucionesProveedor_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                GridViewTemplate gtSalidas = CreateChildTemplateJerarquicoSalidas();
                gtSalidas.Caption = Lenguaje.traduce(strings.Salidas);
                this.rgvGridDevoluciones.Templates.Add(gtSalidas);
        ...
        private void rgvGridDevoluciones_RowSourceNeeded(object sender, GridViewRowSourceNeededEventArgs e)
        {
            try
            {
                ...
                string idDevolPro = string.Empty;
                DataRowView rowView = e.ParentRow.DataBoundItem as DataRowView;
                DataRow[] rows = rowView.Row.GetChildRows("" + Lenguaje.traduce("ID") + "");
                idDevolPro = e.ParentRow.Cells["" + Lenguaje.traduce("ID") + ""].Value.ToString();
                log.Debug("Iniciando row source needed sobre el padre " + idDevolPro);
                ...
                if (e.Template.Caption == Lenguaje.traduce(strings.LineasDevolucion))
                {
                    ...
                    {*/
                        try
                        {
                            DataTable lineasPedido = Business.GetGenericoJerarquicoDatosGridView(nombreJsonJerLineas, "[IDDEVOLPRO]", idDevolPro);
                ...
                else if (e.Template.Caption == Lenguaje.traduce(strings.Entradas))
                {
                    try
                    {
                        DataTable entradasDevolucion = Business.GetGenericoJerarquicoDatosGridView(nombreJsonJerEntradas, "[IDDEVOLPRO]", idDevolPro);
                ...
                else if (e.Template.Caption == Lenguaje.traduce(strings.Existencias))
                {
                    try
                    {
                        DataTable existenciasDevolucion = Business.GetGenericoJerarquicoDatosGridView(nombreJsonJerExistencias, "[IDDEVOLPRO]", idDevolPro);
                ...
                else if (e.Template.Caption == Lenguaje.traduce(strings.Movimientos))
                {
                    try
                    {
                        DataTable movimientosDevolucion = Business.GetGenericoJerarquicoDatosGridView(nombreJsonJerReservas, "[IDDEVOLPRO]", idDevolPro);
                ...
                else if (e.Template.Caption == Lenguaje.traduce(strings.Salidas))
                {
                    try
                    {
                        DataTable tareasDevolucion = Business.GetGenericoJerarquicoDatosGridView(nombreJsonJerSalidas, "[IDDEVOLPRO]", idDevolPro);
        ...
        private void llenarGridDevoluciones(object sender, DoWorkEventArgs e)
        {
            try
            {
                ...
                Business.GetEsquemaGenerico(nombreJson,ref _lstEsquemaTablaDevoluciones);
                dtDevoluciones = Business.GetDatosGridView(nombreJson, "DC.IDDEVOLPRO",_lstEsquemaTablaDevoluciones);               
        ...
        private void rgvGridDevoluciones_CellClick(object sender, GridViewCellEventArgs e)
        {
            if (e.Row.Index > -1 && e.Row.HierarchyLevel == 1)
            {
                ...
                if (template.Caption == Lenguaje.traduce(strings.LineasDevolucion))
                {
                    ...
                    try
                    {                        
                        idDevolProLinSeleccionado = Convert.ToInt32(fila.Cells[Lenguaje.traduce("Línea")].Value);
                    }
                    catch (Exception ex)
                    {
                        log.Info("rgvGridDevoluciones_CellClick no ha encontrado ID Pedido o ID Pedido  Lin");
                       
                        idDevolProLinSeleccionado = 0;
                    }
                }
                else
                {
        ...
        private void cerrarDevolucion(int idDevolPro, int idOperario, int idRecurso)
        {
            ...
            try
            {
                ...
                object[] resultado= ws.cerrarDevolucionPro(idDevolPro,false, DatosThread.getInstancia().getArrayDatos(), idOperario);
            }
            catch (Exception ex)
            {
        ...
        private void rBtnRetirarDevolucion_Click(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;
            List<int> listaDevoluciones = new List<int>();
            List<GridViewRowInfo> listaRecursiva = new List<GridViewRowInfo>();
            ...
            foreach (var id in listaDevoluciones)
            {
                ...
                retirarDevolucion(idDevolucion, idOperario, idRecurso);
            }
        }
        private void retirarDevolucion(int idDevolPro, int idOperario, int idRecurso)
        {
            Cursor.Current = Cursors.WaitCursor;
            WSDevolucionesMotorClient ws = null;
            try
            {
                ws = new WSDevolucionesMotorClient();
               object[] resultado= ws.retirarDevolucionPro(idDevolPro, idOperario);
                if (resultado.Length == 0)
                {
                    RadMessageBox.Show(Lenguaje.traduce("Devolución retirada correctamente"));
                    refrescar();
                }
                else
                {
